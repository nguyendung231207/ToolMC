...
import { storeinfo } from '../bot/botState'
...
export function sendEvent(username, event, message, extra = {}) {
  if (event === 'chat') {
    try {
      const disableChatLog = storeinfo()?.boolean?.disableChatLog
      if (disableChatLog) return
    } catch (e) {
      // safe fallback
    }
  }
  ...
  if (win && !win.isDestroyed()) {
    win.webContents.send('botEvent', info)
  }
}
